...
class ProgressBar:
    def __init__(self, total, desc, device, baseline=None, update_freq=50, color=None, is_eval=False, batch_size=1):
        ...
        if not _is_notebook():
            ...
            if color:
                self.color = color
                self.dev_display = f"{Colors.ORANGE}{device_name}{Colors.RESET}" if is_eval else f"{Colors.GREEN}{device_name}{Colors.RESET}"
...
def log_epoch_summary(epoch: int, epochs: int, train_loss: float, val_loss: float, val_acc: float, improved: bool) -> None:
...
class EpochLogger:
    ...
    def _print_header(self):
        headers = ["Epoch", "Train Loss (imp)", "Val Loss (imp)", "Test Loss (imp)", "Time", "Val Acc", "Test Acc", "LR"] if self.show_accuracy else ["Epoch", "Train Loss (imp)", "Val Loss (imp)", "Test Loss (imp)", "Time", "LR"]
    ...
    def log_epoch(self, epoch, train_loss, val_loss, test_loss, train_ploss, val_ploss, test_ploss, val_acc, test_acc, elapsed, lr, improved, best_val):
        row = [
            f"{epoch + 1}",
